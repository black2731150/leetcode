...
func countValidWords(sentence string) int {
	...
	for _, token := range tokens {
		if first(token) && second(token) && tried(token) {
...
func second(token string) bool {
	...
	for i := 1; i < len(token)-1; i++ {
		if token[i] == '-' {
			if (token[i-1] >= 'a' && token[i-1] <= 'z') && (token[i+1] >= 'a' && token[i+1] <= 'z') && numOfI == 0 {
				numOfI++
				continue
			} else {
				return false
